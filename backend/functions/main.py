...
def chores_3agents(req: https_fn.CallableRequest):
    ...
    is_ef1 = False
    while is_ef1 == False:
        total_disutilities = [sum(valuation_matrix[k][j] for j in x_k) for k, x_k in
                              enumerate(allocation)]  #creates list of size 3
        ...
        if any(item in allocation[b] for item in mpb_l):
        ...
        elif any(item in allocation[h] for item in mpb_l):
            j_ind = 0
            ...
            if earnings(allocation[h], p) - p[j_ind] > earnings(allocation[l], p):
                allocation[l].append(allocation[h].pop(j_ind))

            elif any(item in allocation[b] for item in mpb_h):
                for j in range(len(allocation[b])):
